...
const day1p1 = require('../Day_1_exercise/day1-p1');
...
describe('Day 1-1: No Time for a Taxicab', () => {
  check.test('should return shortest path to the destination', { times: 10 }, gen.oneOf(testDirections), (directions) => {
    const result = day1p1(directions.input.split(', '));
    expect(result).to.equal(directions.output);
  });
});
